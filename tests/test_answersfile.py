...
def test_external_data_files(tmp_path_factory: pytest.TempPathFactory) -> None:
    ...
    build_file_tree(
        {
            ...
            (child / "copier.yml"): (
                ...
                name: "{{ _ext.parent2.child | d(_ext.parent1.child) }}"
                """
            ),
            (child / "combined.json.jinja"): """\
                {
                    "parent1": {{ _ext.parent1.name | tojson }},
                    "parent2": {{ _ext.parent2.name | tojson }},
                    "child": {{ name | tojson }}
                }
            """,
            (child / "ext.json.jinja"): "{{ dict(_ext) | tojson }}",
            (
    ...
    git_save(dst)
    assert json.loads((dst / "ext.json").read_text()) == {
        "parent1": expected_parent1_answers,
        "parent2": expected_parent2_answers,
    }
